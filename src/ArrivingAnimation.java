 ...
 * Description:	A program that turns multiples picture of a plane taking off into an animation.
 */
...
public class ArrivingAnimation extends JPanel {
	private final static String IMAGE_NAME = "arriving-"; // base image name
	protected ImageIcon images[]; // array of images
	private final int TOTAL_IMAGES = 124;  // number of images
	private int currentImage = 0; // current image index
	private final int ANIMATION_DELAY = 25; // millisecond delay
	...
	public ArrivingAnimation() {
		images = new ImageIcon[ TOTAL_IMAGES ];

		// load 48 images
		for ( int count = 0; count < images.length; count++ )
			images[ count ] = new ImageIcon( "arriving/" + IMAGE_NAME + (count+1) + ".png" );
